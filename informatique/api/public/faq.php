...
<div class="h-full py-20 flex flex-col justify-top">
    ...
    <div class="pt-10 w-full flex justify-center">
        <div class="max-w-[650px] w-full flex flex-col gap-5 items-center justify-center">
            <input id="searchInput" class="w-full h-14 px-4 py-4 border rounded-3xl border-gray-200 focus:outline-none focus:ring focus:border-eventit-500" type="text" placeholder="Rechercher une réponse" data-lang="Rechercher une réponse|Search for an answer"/>
            <div class="flex gap-3 justify-center w-full flex-wrap">
            <?php foreach ($sections as $section) : ?>
    <button class="w-full md:w-[20%] text-lg px-4 py-2 border rounded-3xl bg-eventit-500 text-white focus:outline-none focus:ring focus:border-eventit-500 section-button" data-lang="<?php 
...
<?php endforeach; ?>
                <button class="w-full md:w-[20%] text-lg px-2 py-2 border rounded-3xl bg-eventit-500 text-white focus:outline-none focus:ring focus:border-eventit-500 section-button" id="showAllButton" data-lang="Voir tout|See all">Voir tout</button>
            </div>
            <div class="w-full flex flex-col gap-5 accordion-container">
                <?php foreach ($questions as $question) : ?>
                    <div class="accordion border rounded-3xl border-eventit-500" data-section="<?php echo $question->section; ?>">
                        <button class="text-left w-full flex px-4 py-4">
                            ...
                            <svg class="ml-auto arrow h-6 w-6 transform text-eventit-500 transition-transform duration-200" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M10 18a.75.75 0 01-.53-.22l-7-7A.75.75 0 013.22 9.47L10 16.25l6.78-6.78a.75.75 0 111.06 1.06l-7 7a.75.75 0 01-.53.22z" clip-rule="evenodd" />
                            </svg>
                        </button>
