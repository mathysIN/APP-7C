...
<div class="h-full py-20 flex flex-col justify-top">
    ...
    <div class="pt-10 w-full flex justify-center">
        <div class="max-w-[650px] w-full flex flex-col gap-5 items-center justify-center">
            <input id="searchInput" class="w-full h-14 px-4 py-4 border rounded-3xl border-gray-200 focus:outline-none focus:ring focus:border-eventit-500" type="text" placeholder="Rechercher une réponse" />
            <div class="flex gap-3 justify-center w-full flex-wrap">
                <?php foreach ($sections as $section) : ?>
                    <button class="w-full md:w-[20%] text-lg px-4 py-2 border rounded-3xl bg-eventit-500 text-white focus:outline-none focus:ring focus:border-eventit-500 section-button" data-section="<?php echo $section; ?>">
                        <?php echo ($section === 'event-it') ? 'Event-it' : (($section === 'devis') ? 'Devis' : (($section === 'compte') ? 'Compte' : (($section === 'capteurs') ? 'Capteurs' : $section))); ?>
                    </button>
                <?php endforeach; ?>
                <button class="w-full md:w-[20%] text-lg px-2 py-2 border rounded-3xl bg-eventit-500 text-white focus:outline-none focus:ring focus:border-eventit-500 section-button" id="showAllButton">Voir tout</button>
            </div>
